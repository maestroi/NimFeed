...
describe('Nimiq Pay write controls', () => {
  it('shows the post button without hiding Home or Search', () => {
    const nav = source('src/components/layout/BottomNav.vue')

    expect(nav).toMatch(
      /v-if="walletRuntime\.canPublishPosts\.value"\s+type="button"\s+class="nf-focus nf-press mx-auto h-12 w-12/,
    )
